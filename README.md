...
В прототипе также показана работа с разнородными источниками данных через интерфейс `DataSourceProvider`: один отчёт использует данные из PostgreSQL, второй — из внешнего источника (mock external API).
...
4. Запустите фронтенд локально:
   ...
   npm start
   ```
  Фронтенд будет доступен на http://localhost:3000 и автоматически подключится к бэкенду по `REACT_APP_API_URL` (по умолчанию `http://localhost:3001/api`).
...
### Источники данных в прототипе

- `Sales` отчёты: источник `PostgreSQL` (через Prisma)
- `Analytics` отчёты: источник `mock external API` (заглушка внешнего HTTP API)

Выбор источника выполняется в worker через `DataSourceProvider` (`backend/src/dataSources/reportDataSources.ts`), что демонстрирует расширяемый паттерн для подключения новых источников.
